...
- [x] Build it: `/develop better auth email password`
   - [x] Schema replace + Prisma generate and migrate (AC-1, AC-9)
   - [x] Better Auth config + Nest AuthModule wiring (AC-1, AC-2, AC-3)
   - [x] `GET /users/me`, anonymous `GET /`, sign out path (AC-4, AC-5, AC-6, AC-8)
   - [x] ADMIN seed or script + end to end smoke (AC-7, AC-1..9)
- [ ] Verify it: `/check verify better auth email password`
- [ ] Test it: `/test better auth email password`
Spec [0001](../specs/0001-better-auth-email-password.md) · code in `src/auth/`, `src/users/`, `prisma/`
